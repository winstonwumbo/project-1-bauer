...
const createPolyfillsLoaderConfig_js_1 = require("./createPolyfillsLoaderConfig.js");
function polyfillsLoader(pluginOptions = {}) {
    ...
    return {
        ...
        buildStart(options) {
            ...
            if (!options.plugins) {
                throw (0, utils_js_1.createError)('Could not find any installed plugins');
            }
            const htmlPlugins = options.plugins.filter(p => p.name === '@web/rollup-plugin-html');
            const [htmlPlugin] = htmlPlugins;
            if (!htmlPlugin) {
                throw (0, utils_js_1.createError)('Could not find any instance of @web/rollup-plugin-html in rollup build.');
            }
            htmlPlugin.api.disableDefaultInject();
            htmlPlugin.api.addHtmlTransformer(async (html, { bundle, bundles, htmlFileName }) => {
                var _a, _b, _c;
                const config = (0, createPolyfillsLoaderConfig_js_1.createPolyfillsLoaderConfig)(pluginOptions, bundle, bundles);
                ...
                let htmlString = html;
                if ((0, utils_js_1.shouldInjectLoader)(config)) {
                    const result = await (0, polyfills_loader_1.injectPolyfillsLoader)(html, Object.assign(Object.assign({}, config), { relativePathToPolyfills }));
                ...
                for (const entrypoint of entrypoints) {
                    const importPath = normalize(path_1.default.posix.relative('', entrypoint.importPath));
                    preloaded.push(importPath);
                    // js files (incl. chunks) will always be in the root directory
                    const pathToRoot = path_1.default.posix.dirname(importPath);
                    for (const chunkPath of entrypoint.chunk.imports) {
                        const relativeChunkPath = normalize(path_1.default.posix.join(pathToRoot, chunkPath));
                        preloaded.push(relativeChunkPath);
                    }
                }
                preloaded = [...new Set(preloaded)];
                const type = (_b = (_a = pluginOptions.modernOutput) === null || _a === void 0 ? void 0 : _a.type) !== null && _b !== void 0 ? _b : (0, createPolyfillsLoaderConfig_js_1.formatToFileType)((_c = bundle === null || bundle === void 0 ? void 0 : bundle.options.format) !== null && _c !== void 0 ? _c : 'esm');
